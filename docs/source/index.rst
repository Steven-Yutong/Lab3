...
read方法并没有遵循单一职责原则。read方法中包含了判断用户是否合法、判断文章是否在库内和为用户提供文章进行阅读三个方法，而前两种方法并不是read方法的主体方法，即其方法的具体实现逻辑、内容不应该是read方法需要了解的；当判断用户是否合法或是判断文章是否在库内的需求改变，则需要对read整个方法进行修改、测试，大大降低了维护的效率。
